...
{
	TriangleMesh * mesh = new TriangleMesh();
	mesh->load(filename, Matrix4x4(Vector4(1,0,0,0), Vector4(0,1,0,0), Vector4(0,0,1,0), Vector4(position.x, position.y, position.z, 1)));
...
//	bunnyMat->SetReflection(0.25f);
    ...
    sphere->setRadius(1.5);
    sphere->setMaterial(new Phong(Vector3(1.0f, 0.5f, 0.25f), Vector3(0.1, 0.1, 0.1), Vector3(1, 1, 1)));
    ...
    sphere->setMaterial(new TexturedPhong(new StoneTexture(20), Vector3(0.1,0.1,0.1), Vector3(1,1,1)));
    //g_scene->addObject(sphere);
    ...
    Material* teapotMat = new Phong(Vector3(.5f, .5f, .5f), Vector3(0.1, 0.1, 0.1), Vector3(0, 0, 0));
